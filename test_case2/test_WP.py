...
class TestWP(StartEnd):
    ...
    # @unittest.skip('skip test_wp_print_all_pic')
    def test_wp_print_all_pic(self):
        logging.info('==========%s==========' % self.__str__().split(' ')[0])
        wp = WPView(self.driver)
        file_name = '欢迎使用永中Office.docx'
        search_result = wp.search_file(file_name)
        ...
        wp.group_button_click('文件')
        s = wp.swipe_option('up')
        wp.while_not_exist_ele_swipe('//*[@text="逐页输出图片"]', s)
        wp.get_element('//*[@text="逐页输出图片"]').click()
        self.assertTrue(wp.exist('//*[@text="图片预览"]') and wp.exist('//*[@text="取消全选"]'),
                        msg='未弹出图片预览窗口')

        wp.get_element('//*[@text="取消全选"]').click()
        self.assertTrue(wp.exist('//*[@text="全选"]'),
                        msg='取消全选后，未显示全选按钮')
        wp.get_element('//*[@text="全选"]').click()
        wp.get_element('//*[@text="保存到相册"]').click()
        time.sleep(10)
        wp.get_element('//*[@text="分享"]').click()
        time.sleep(10)
        self.assertTrue(wp.exist('//*[@text="微信"]') and wp.exist('//*[@text="QQ"]')and wp.exist('//*[@text="钉钉"]')and wp.exist('//*[@text="邮箱"]'),
                        msg='未弹出分享栏')
